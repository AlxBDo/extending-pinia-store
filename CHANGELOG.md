...
### Added
- `createParentStore`function that allows you to define a new entry in the `parentsStores` array.

### Changed
- `parentsStores` option accepts an array or an object.


## [v0.1.3] - 2026-09-01

### Fixed
- **Regression from v0.1.2 (real root cause)**: fixed `vite.config.ts`'s `rollupOptions.external`, which listed `'pinia-plugin-subscription'` as an exact string. Since v0.1.2, `StoreExtension.ts` and `ExtendsPiniaStore.ts` import from the package's `pinia-plugin-subscription/helpers` subpath (and `/types` for types), but Rollup's external matching only matches exact specifiers by default — subpaths are not automatically covered. As a result, `pinia-plugin-subscription`'s `Store`, `Debug`, `PluginSubscriber` classes and its module-level `defineAStoreSetupContexts`/`defineAStoreSetupContextsById` singleton maps were silently inlined/duplicated into the built bundle instead of being resolved to the consumer's single shared copy of `pinia-plugin-subscription`. This produced two independent instances of the setup-context registry: `setEnhancedStore` (called through the app's own `pinia-plugin-subscription` import) wrote to one instance while `StoreExtension`'s internal `Store` base class (using the inlined copy) read from another, so `getEnhancedStore` always failed with `Error: enhanced store is required` — most visibly under SSR (e.g. Nuxt), where the mismatch is exercised on every request. `external` now matches `pinia-plugin-subscription` and all its subpaths (`/^pinia-plugin-subscription(\/.*)?$/`), and `output.globals` was updated accordingly so the UMD build also resolves an explicit global name for each subpath. No source logic changed; this is a build-configuration-only fix that restores exact v0.1.1 bundling behavior (nothing from `pinia-plugin-subscription` bundled, regardless of entry point).
- Reverted `StoreExtension.extendsAction` to its pre-v0.1.2 synchronous, fire-and-forget behavior. The v0.1.2 rewrite added a proactive `TypeError` thrown synchronously during store construction when the parent/child action wasn't (yet) a function, and introduced promise-based chaining (awaiting the parent action before running the child, propagating parent errors, returning the child's result). In real apps where the child store's own action isn't assigned yet at the moment `StoreExtension` builds the store (e.g. actions provided by another Pinia plugin not yet applied), this made the constructor throw. The action is now wrapped exactly as in v0.1.1: parent action runs, then child action runs right after, synchronously, with no promise awaiting, no proactive type checks, and no return value.
